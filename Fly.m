...
function self = findangle(self)
    ...
    self.size5angle1st = abs(diff(self.size5angle(:)));%first derivative of angle
    self.size5angle1stavg = mean(self.size5angle1st(:));
    self.size5angle2nd = abs(diff(self.size5angle1st(:)));
    self.size5angle2ndavg = mean(self.size5angle2nd(:));
end
...
function plotdiagrams(self)
    ...
    self.size10angle = abs(self.size10angle);
    figure;
    histogram(self.size5steps,'Normalization','probability');
    title('Size 5 steps distribution');
